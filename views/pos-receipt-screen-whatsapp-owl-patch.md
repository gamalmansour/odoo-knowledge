...
- **UltraMsg Dual-Dispatch & Duplicate Send Trap:** In UltraMsg Gateway mode, sending via `/messages/chat` (text) followed by `/messages/image` (picture) delivers two separate messages to the customer's phone. Consolidate them into a single `/messages/image` call with the complete bill formatted inside `caption` (up to 1024 chars). Furthermore, prevent concurrent double dispatches (auto-send on screen mount + manual cashier click) by adding an in-memory 15s debounce guard on `pos.order` in Python, tracking `state.whatsappSent`, and disabling double-invocation in OWL.
