...
- **Python Falsy Zero Fallback Hazard (`0.0 or target_field`):** When computing actual performance metrics (such as `closed_tcr`), never write fallback expressions like `f.closed_tcr or f.transactions_month`. In Python, `bool(0.0) == False`. If a salesperson closed zero deals in a month, `0.0 or 3.0` evaluates to `3.0`, falsely attributing target quotas as actual completed sales! Actual performance counters must strictly use `(f.closed_tcr or 0.0)`.
