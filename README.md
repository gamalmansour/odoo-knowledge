...
| 38 | [stored-compute-parent-stale-sequential-child-create.md](orm/stored-compute-parent-stale-sequential-child-create.md) | 🔴 Critical | 17 | `orm`, `computed-fields`, `store`, `one2many`, `import`, `hierarchy` | Stored editable compute aggregating one2many children freezes at a partial sum when children are created row-by-row after the parent (import wizards) — recompute parents explicitly after the hierarchy exists |
